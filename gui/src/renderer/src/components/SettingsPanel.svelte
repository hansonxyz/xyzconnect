<script lang="ts">
  ...
  function updateStatusText(status: UpdateStatus): string {
    switch (status.state) {
      ...
      case 'downloaded': return `Version ${status.version} ready to install`
  ...
  const stateLabel = $derived(
    hasEverConnected ? 'Connected' : hasPairedDevice ? 'Reconnecting' : 'Disconnected',
  )
...
<div class="settings-panel">
  <div class="settings-panel__header">
    ...
    <button class="settings-panel__close" onclick={onClose} title="Close settings">
  ...
  <div class="settings-panel__content">
    <div class="settings-panel__section">
      <h3 class="settings-panel__section-title">Connection</h3>

      <div class="settings-panel__row">
        <span class="settings-panel__label">Status</span>
        <span class="settings-panel__value">{stateLabel}</span>
      </div>

      {#if cachedName}
        <div class="settings-panel__row">
          <span class="settings-panel__label">Device</span>
          <span class="settings-panel__value">{cachedName}</span>
        </div>
        {#if cachedAddress}
          <div class="settings-panel__row">
            <span class="settings-panel__label">IP Address</span>
      ...
      {:else if hasPairedDevice}
        <div class="settings-panel__row">
          <span class="settings-panel__label">Device</span>
          <span class="settings-panel__value settings-panel__value--muted">Waiting for device...</span>
        </div>
      {/if}
    </div>

    <div class="settings-panel__section">
      <h3 class="settings-panel__section-title">Notifications</h3>
      <div class="settings-panel__row">
        <span class="settings-panel__label">Desktop notifications</span>
      ...
      <div class="settings-panel__row">
        <span class="settings-panel__label">Flash taskbar on new message <span class="settings-panel__hint">(Windows)</span></span>
        <label class="settings-panel__toggle">
          <input
            type="checkbox"
    ...
    <div class="settings-panel__section">
      <h3 class="settings-panel__section-title">Updates</h3>

      <div class="settings-panel__row">
        <span class="settings-panel__label">Version</span>
        <span class="settings-panel__value settings-panel__value--mono">{appVersion}</span>
      </div>

      <div class="settings-panel__row">
        <span class="settings-panel__label">Check automatically</span>
        <label class="settings-panel__toggle">
    ...
    {#if devices.pairedIds.length > 0}
      <div class="settings-panel__section">
        <h3 class="settings-panel__section-title">Device</h3>

        {#if confirmingUnpair}
          <div class="settings-panel__confirm">
            <p class="settings-panel__confirm-text">
              Unpair from {cachedName ?? 'this device'}? You'll need to pair again to use XYZConnect.
            </p>
